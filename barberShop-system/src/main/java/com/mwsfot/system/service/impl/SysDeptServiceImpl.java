    ...
    public void checkDeptTenantAllowed(SysDept dept) {
        if ( dept.getTenantId() == null ) {
            ...
            if ( !TenantConstants.DEFAULT_SYSTEM_TENANT_ID.equals(SecurityUtils.getTenantId()) ) {
                SysDept query = selectDeptById(dept.getDeptId());
                if ( !SecurityUtils.getTenantId().equals(query.getTenantId()) ) {
                    throw new ServiceException("不允许操作非自身租户平台部门");
                }
            }

        } else {
            //如果租户ID和默认系统ID相同则允许操作
            if ( !TenantConstants.DEFAULT_SYSTEM_TENANT_ID.equals(dept.getTenantId()) ) {
                if ( !SecurityUtils.getTenantId().equals(dept.getTenantId()) ) {
                    throw new ServiceException("不允许操作非自身租户平台部门");
                }
            }
        }
