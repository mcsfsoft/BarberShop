  ...
  <div class="app-container">
          ...
          <el-col>
            <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch"
              ...
              <el-form-item label="登录名称" prop="userName">
                <el-input v-model="queryParams.userName" placeholder="请输入登录名称" clearable style="width: 240px"
            ...
            <el-row :gutter="10" class="mb8">
              ...
              <!--              <el-col :span="1.5">
                              <el-button type="success" plain icon="el-icon-edit" size="mini" :disabled="single"
                                         @click="handleUpdate(null)" v-hasPermi="['system:user:edit']">修改
                              </el-button>
                            </el-col>-->
              <el-col :span="1.5">
              ...
              <el-col :span="1.5">
                <el-button type="info" plain
                           icon="el-icon-upload2" size="mini" @click="handleImport" v-hasPermi="['system:user:import']">
                  导入
                </el-button>
              </el-col>
              <el-col :span="1.5">
            ...
            <el-table v-loading="loading" :data="userList" @selection-change="handleSelectionChange">
              <el-table-column type="selection" width="50" align="center" :selectable="disableAdmin"></el-table-column>
              <el-table-column label="用户编号" align="center" key="userId" prop="userId" v-if="columns[0].visible"/>
              <el-table-column label="登录名称" align="center" key="userName" prop="userName" v-if="columns[1].visible"
    ...
    <el-dialog :title="upload.title" :visible.sync="upload.open" width="400px" append-to-body>
      <el-upload ref="upload" :limit="1" accept=".xlsx, .xls" :headers="upload.headers"
                 :action="upload.url + '?updateSupport=' + upload.updateSupport + '&tenantId=' + upload.tenantId"
...
export default {
  ...
  components: {Treeselect, Splitpanes, Pane, getters},
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
     ...
     * @param row 当前行
     * @param index 下标
     * @return {boolean} true 可选择 false 不可选择
     */
    disableAdmin(row, index) {
      return !row.admin && row.isFix === 0;
    },
    /** 查询用户列表 */
    getList() {
      this.loading = true;
    ...
    /** 查询部门下拉树结构 */
    getDeptTree() {
      deptTreeSelect({'tenantId': this.queryParams.tenantId}).then(response => {
    ...
    handleAdd() {
      this.reset();
      const params = {'tenantId': this.queryParams.tenantId};
    ...
    handleUpdate(row) {
      this.reset();
      const userId = row ? row.userId : this.ids[0];
      const params = {'tenantId': this.queryParams.tenantId, 'userId': userId};
      getUser(params).then(response => {
    ...
    handleAuthRole: function (row) {
      if (row.tenantId) {
        getTenantAuthRole(row.tenantId).then(res => {
          if (!res.data || res.data === 0) {
            this.$modal.msgError("当前用户平台不存在可使用角色,请先添加角色");
          } else {
            const userId = row.userId;
            this.$router.push("/system/user-auth/role/" + userId + '/' + row.tenantId);
          }
        }).catch(e => {
          console.error(e)
        })
    ...
    handleImport() {
      ...
      this.upload.tenantId = this.queryParams.tenantId
...
//平台端选择框宽度调整
div.el-select.el-select--medium {
  width: 100%;
}
</style>