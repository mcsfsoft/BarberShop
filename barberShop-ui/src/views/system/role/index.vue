  ...
  <div class="app-container">
    <el-row :gutter="20">
      ...
      <el-table v-loading="loading" :data="roleList" @selection-change="handleSelectionChange">
        <el-table-column type="selection" width="55" align="center" :selectable="disableAdmin"/>
...
<!--            <el-tooltip content="控制器中定义的角色编码，如：@PreAuthorize(`@ss.hasRole('admin')`)" placement="top">
          ...
          <el-input v-model="form.roleKey" placeholder="请输入角色编码" disabled/>
        </el-form-item>
...
export default {
  ...
  watch: {
    ...
    'form.roleName'() {
      if (this.form.roleName) {
        this.form.roleKey = pinyin.getCamelChars(this.tenantList.find(item => item.tenantId === this.queryParams.tenantId).tenantName + '_' + this.form.roleName)
      }
    }
  },
  created() {
  ...
  methods: {
    ...
    /** 查询菜单树结构 */
    getMenuTreeselect() {
      menuTreeselect({tenantId: this.queryParams.tenantId}).then(response => {
    ...
    handleAuthUser: function (row) {
      ...
      if (tenantId === this.$store.getters.tenantId || this.$store.getters.tenantId === '1') {
        this.$router.push({name: 'AuthUser', params: {roleId: roleId, tenantId: tenantId}});
      }
    },
    /** 提交按钮 */
    submitForm: function () {
